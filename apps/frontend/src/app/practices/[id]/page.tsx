...
export default function PracticeDetailPage() {
  ...
  // Detectar la posición del scroll en el contenedor
  const handleScroll = () => {
  ...
  // Carga de práctica permitiendo actualización silenciosa (isInitial = false)
  const loadPractice = useCallback(async (isInitial = false) => {
    if (!id || !user) return;
    ...
    try {
      ...
      showToast('error', err instanceof Error ? err.message : 'Error al cargar la práctica');
    } finally {
  ...
  // Hacer scroll inicial al abrir la pestaña de notas
  useEffect(() => {
    if (activeTab === 'notes') {
  ...
  const handleSaveDetails = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPractice(updated);
      setIsEditing(false);
      showToast('success', 'Ficha de práctica actualizada correctamente.');
    } catch (err: unknown) {
      showToast('error', err instanceof Error ? err.message : 'Error al guardar los cambios');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleAddNote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('success', 'Nota agregada a la bitácora.');
      
      await loadPractice(false);
      scrollToBottom(true);
    } catch (err: unknown) {
      showToast('error', err instanceof Error ? err.message : 'Error al registrar la nota');
    } finally {
  ...
  const handleDeleteNote = async (noteId: string) => {
    try {
      ...
      showToast('error', err instanceof Error ? err.message : 'Error al eliminar nota');
    }
  };
  ...
  const handleUpdateNote = async (noteId: string, content: string) => {
    try {
      ...
      showToast('error', err instanceof Error ? err.message : 'Error al actualizar nota');
    }
  };
  ...
  const handleUpdateStatus = async (newStatus: PracticeStatus) => {
    try {
      ...
      setPractice(updated);
      showToast('success', `Estado actualizado a ${newStatus === 'FINISHED' ? 'FINALIZADA' : 'EN PROGRESO'}.`);
      await loadPractice(false);
    } catch (err: unknown) {
  ...
  const handleAssignTeacher = async (teacherId: string) => {
    try {
      ...
      setPractice(updated);
      showToast('success', teacherId ? 'Docente asignado.' : 'Docente desasignado.');
      await loadPractice(false);
    } catch (err: unknown) {
      showToast('error', err instanceof Error ? err.message : 'Error al asignar docente');
    }
  };
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      {activeTab === 'notes' && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col relative">
          ...
          <div 
            className="p-6 space-y-4 max-h-[600px] overflow-y-auto relative" 
            ref={scrollRef}
          ...
          {/* Botón flotante de nuevos mensajes estilo WhatsApp/Slack */}
          {unreadCount > 0 && (
            <div className="absolute bottom-20 left-1/2 -translate-x-1/2 z-10">
              <button
                onClick={() => scrollToBottom(true)}
                className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-4 py-2 rounded-full shadow-lg transition flex items-center space-x-2 animate-bounce"
              >
                <span>↓ {unreadCount} {unreadCount === 1 ? 'nuevo mensaje' : 'nuevos mensajes'}</span>
              </button>
            </div>
          )}
